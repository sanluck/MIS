#!/usr/bin/python
# -*- coding: utf-8 -*-
# dvn-f2_2.py - формирование маршрутных карт ДВН
#l              для clinical_checkup_id,
#l              которых нет в clinical_checkup_results
#
...
def register_ccr(dbmy, cc_id):
    ...
    VALUES
    ({0}, '{1}');"""
    s_sql = s_sqlt.format(cc_id, sdnow)
...
def get_cclist(db):
    s_sqlt = """SELECT 
c.clinical_checkup_id, c.clinic_id_fk, c.people_id_fk
FROM clinical_checkups c
WHERE 
(c.date_end_1 is NOT NULL) AND (c.people_status_code in (3,6))
AND c.clinical_checkup_id NOT IN
	(SELECT DISTINCT cr.clinical_checkup_id_fk FROM clinical_checkup_results cr)
ORDER BY c.clinic_id_fk;"""
    s_sql = s_sqlt
...
if __name__ == "__main__":
    ...
    log.info('============= DVN-F2_2 =====================================================')
    log.info('DVN Missing Clinical Checkups Results Processing Start {0}'.format(localtime))
    ...
    localtime = time.asctime( time.localtime(time.time()) )
    log.info('DVN Missing Clinical Checkups Results Processing Finish  '+localtime)
