#!/usr/bin/python
# -*- coding: utf-8 -*-
# reso-4-2.py - выбор из KISSMO карт enp$card
#             у которых прична закрытия = 2
#
...
s_sqlt_enp = """SELECT s.enp 
FROM erz$s_file s 
WHERE s.reason = ? AND s.dstop >= ?;"""
...
def get_enp_list():
    ...
    ro_cur.execute(s_sqlt_enp, (REASON, D_START, ))
    results = ro_cur.fetchall()
    
    sout = "REASON: {0} D_START: {1}".format(REASON, D_START)
    log.info(sout)
    ...
    sout = "We have got {0} ENPs matching criteria".format(nnn)
    log.info(sout)
    
    return enplist

def get_plist(enplist):
    import fdb
    ...
    for rec in results:
        ...
        if not enp in enplist: continue
    ...
    sout = "We have got {0} out of {1} records".format(nnf, nnn)
    log.info(sout)
